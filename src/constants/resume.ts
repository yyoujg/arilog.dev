...
export const RESUME: Resume = {
  ...
  experience: [
    {
      company: COMPANY,
      role: "운영 백오피스(객실·패키지·판매채널 관리) · 프론트엔드 단독",
      period: "2026.07 ~ 재직중",
      description: [
        "여러 운영자가 같은 객실·패키지를 동시에 수정하는 호텔 운영 백오피스. 프레임워크 세팅부터 프론트엔드를 단독으로 담당하고 있습니다.",
        "목업 데이터로만 동작하던 화면을 인증 → BFF 프록시·라우트가드 → 화면 연동 순으로 실제 API에 연결. 배포 후 프록시가 빈 500을 내던 원인을 nginx를 거치며 붙는 hop-by-hop 헤더로 특정해 전달 전 제거",
        "백엔드가 낙관적 잠금(수정 시각 대조)을 필수로 바꾸며 쓰기 경로가 전부 400이 된 상황에서, 기준값을 요청 타입의 필수 필드로 지정해 호출처 누락을 컴파일 단계에서 차단. 쓰기 응답과 조회 응답의 타임스탬프 정밀도가 달라 두 번째 저장이 곧바로 충돌하던 문제를 발견해 백엔드와 정렬",
        "일괄 수정은 1건 충돌로 전체가 막히지 않도록 부분 성공을 허용하고, 실패 대상·사유를 결과 팝업으로 표시(기획 확인 후 반영)",
        "공통 모달의 z-index를 마운트 순번이 아닌 현재 열린 깊이 기준으로 바꿔, 닫힌 모달이 번호를 소모해 드롭다운을 덮던 문제 해소",
        "테스트 시트 512케이스 중 441건 화면 테스트 수행, 발견 30건 반영·재검증. 동시 요청 재현으로 판매채널 저장의 잠금 결함을 찾아 백엔드 수정 후 재검증",
    ...
    },
    {
      ...
      description: [
        ...
        "예약 식별정보를 AES-256-GCM 봉인 토큰(짧은 수명 · 용도별 분리)으로 인계, URL 쿼리 노출 제거",
        ...
        "성능 측정값이 없던 예약 페이지에 Lighthouse 합성 측정 기준선(4개 경로)을 세우고, 레저 상세 CLS 0.440의 94%가 히어로 스켈레톤과 실제 갤러리의 박스 불일치에서 온다는 것을 특정 → 동일 박스로 교체해 동일 조건 재측정 CLS 0.074",
      ],
    },
    {
      company: COMPANY,
      ...
      description: [
        ...
        "컴포넌트 37종(현재 0.17.0) — 초기 8종(Button·Input·Dropdown·Badge·Modal·Toast·Tooltip·Popover)에서 예약 페이지·백오피스 컴포넌트로 확장",
        "예약 앱에 도입하자 lg: 반응형이 깨져 운영에서 롤백됐던 문제를, 빌드 산출 CSS에서 DS 유틸리티가 앱 미디어 쿼리 뒤에 놓이는 캐스케이드 충돌로 규명. 앱 Tailwind가 DS 클래스를 직접 생성하는 방식(@source)으로 바꿔 우회용 ! 오버라이드 116건을 제거하고 운영에 재도입",
        "스토리북 CI가 develop 전용이라 MR에 파이프라인이 생기지 않아 머지가 막히던 문제, 브라우저 캐시 키가 lockfile에 묶여 러너 디스크가 차던 문제를 정리",
    ...
    {
      company: COMPANY,
      role: "B2C 예약 서비스(별도 코드베이스) · 부킹엔진 예약 검색·날짜 선택 담당 / CRS 어드민 판매·상품 화면 설계",
      period: "2024.04 ~ 2024.07 · 2024.08 ~ 2025.03 · 2026.07 ~ 2026.08",
      description: [
        "주력 서비스와는 별도 레포·별도 배포 파이프라인을 쓰는 B2C 예약 사이트에서 부킹엔진 검색·날짜 선택과 CRS 어드민 판매·상품 화면을 담당했습니다.",
